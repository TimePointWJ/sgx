...
public class NewsServiceImpl implements NewsService {
	...
	public Object insertNews(ViewNews viewNews, UnionUser user) {
		 ...
		 */
		if (viewNews.getFj() != null && viewNews.getFj().length() != 0) {
			UnionSource source = new UnionSource();
			source.setCreatTime(new Date());
			source.setDelflag("A");
			source.setAttachmentSource("02");
			source.setSourceId(viewNews.getId());
			source.setFileName(viewNews.getFj());
			source.setFileUrl("D\\ynw\\" + viewNews.getFj());
			unionSourceMapper.insertSource(source);
		}
	...
	public Object updateNews(ViewNews viewNews) {
		...
		/*
		 * List<UnionSource> list = viewNews.getSourceList(); if(list!=null &&
